def list_bones(asset_path: str, name_filter: str = "", max_results: int = 100):
    """List all bones in a Control Rig with transforms and hierarchy.

    Returns bone names, parent-child relationships, and initial transforms.

    :param asset_path: Full content path to the Control Rig (e.g. "/Game/CR_MyRig").
    :param name_filter: Optional substring filter on bone name (case-insensitive).
    ...
    try:
        ...
        return {
            "success": False,
            "message": "Failed to list bones: {}".format(str(e)),
        }

    total_count = len(bones)
    bones = bones[:max_results]
